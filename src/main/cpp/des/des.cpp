...
//  INFO("des_crypt cryptMode:%d, key:%s", cryptMode, key);
  if (size <= 0) {
    WARN("des_crypt src size <= 0");
    *outDst = nullptr;
    *outSize = 0;
    return;
  }
  if (nullptr == key || strlen((const char *)key) < 8) {
    WARN("des_crypt key len < 8");
    *outDst = nullptr;
    *outSize = 0;
    return;
  }
  ...
  unsigned int block = size / 8;
  if (DES_ENCRYPTION_MODE == cryptMode) {
    // 加密多增加一个
    block += 1;
  }
  size_t remainders = size % 8;
  size_t padding = 8 - remainders;

  *outSize = block * 8;

  *outDst = (unsigned char *)malloc(*outSize);
  memset(*outDst, 0, *outSize);
...
//      INFO("des_crypt last one");
      memcpy(buf_src, src + i * 8, size - (i * 8));
    }
    if (i != block - 1) {
      // 非padding
      process_message((unsigned char *) buf_src,
                      (unsigned char *) buf_dst,
                      key_sets,
                      cryptMode);
//      INFO("des_crypt i:%d is :%02x%02x%02x%02x%02x%02x%02x%02x", i, buf_dst[0],buf_dst[1],buf_dst[2],buf_dst[3],buf_dst[4],buf_dst[5],buf_dst[6],buf_dst[7]);
      memcpy(*outDst + (i * 8), buf_dst, 8);
    } else if (DES_ENCRYPTION_MODE == cryptMode) {
      // 加密的padding
      memset(buf_src + 8 - padding, padding, padding);
      process_message((unsigned char *) buf_src, (unsigned char *) buf_dst, key_sets, cryptMode);
//      INFO("des_crypt last one is :%02x%02x%02x%02x%02x%02x%02x%02x", buf_dst[0],buf_dst[1],buf_dst[2],buf_dst[3],buf_dst[4],buf_dst[5],buf_dst[6],buf_dst[7]);
      memcpy(*outDst + (i * 8), buf_dst, 8);
    } else {
      // 解密的padding，关键是需要再调整长度
      process_message((unsigned char *) buf_src, (unsigned char *) buf_dst, key_sets, cryptMode);
      memcpy(*outDst + (i * 8), buf_dst, 8);
      int decPadding = buf_dst[7];
//      INFO("des_crypt padding:%d", decPadding);
      if (decPadding >= 0 && decPadding <= 8) {
        // 解密时删除尾部的padding
        *outSize = *outSize - decPadding;
      }
    }
